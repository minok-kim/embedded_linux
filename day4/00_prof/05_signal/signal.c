...
int main(int argc, char **argv)
{
    ...
    else if(pnum==2)
    {
        ...
        while(1)
        {
            ...
            printf("Error level | before : %d  after : %d\n",err_rate_b,err_rate_a);
            sleep(1);
        }
    }
    else if(pnum==3)
    ...
    else if(pnum==4)
    {
        while(1)
        {
            if(GPIO_GET(g_in1)>0 && GPIO_GET(g_in2)==0)
            {
                kill(pid1,SIGSTOP);
                printf("Input Process Stop\n");
                sleep(1);
            }
            else if(GPIO_GET(g_in1)==0 && GPIO_GET(g_in2)>0)
            {
                kill(pid1,SIGCONT);
                printf("Input Process Continue\n");
                sleep(1);
            }
        }
    }
